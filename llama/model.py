    ...
    def forward(
        ...
        if return_attn:
            h, attn = self.attention(
                self.attention_norm(x), start_pos, freqs_cis, mask, return_attn
            )
        else:
            h = self.attention(self.attention_norm(x), start_pos, freqs_cis, mask)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        Tensor, Tuple[Tensor, List[Tensor]], Tuple[Tensor, List[Tensor], List[Tensor]]
    ]:
        ...
        for layer in self.layers:
            if return_attn:
                h, attn = layer(h, start_pos, freqs_cis, mask, return_attn)
        ...
        if return_hidden and return_attn:
            return output, hidden_output, attn_output
        if return_hidden:
